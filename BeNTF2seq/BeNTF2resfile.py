...
def get_SecBulgePos_here(bp,BeNTF2_obj):
...
def ProduceResfileLines(POSE,BeNTF2_obj):
	lines = []
	...
	if BeNTF2_obj.ring_obj.sheet_obj.Second_bulge_E3:
		SecBulge_A = get_SecBulgePos_here(bp,BeNTF2_obj); SecBulge_A_aa = 'EDH'
	...
	for key in bp.segment_dict.keys():
		if (key[0] == 'L') and (len(key) == 2) :
			...
			if abego == 'GG':
				...
				lines.append( '%d A PIKAA N\n'%(pos_1st) )
			elif abego == 'EA':
				pos_2nd = bp.segment_dict[key].bp_data[1][0]
				lines.append( '%d A PIKAA D\n'%(pos_2nd) )
...
if __name__ == "__main__":
	...
	for line in ProduceResfileLines(POSE,BeNTF2_obj): resfile_handle.write(line)
